...
- 生产根页面与登录页只读检查均返回 200；生产后台生成接口鉴权验证通过并返回 `idle`，当前没有待生成任务。
- 后台 Scheduler 保持暂停，避免在新版 Sites 发布前请求尚不存在的生产接口。
- 使用指南仍是临时文字版；用户尚未提供最终页面原型。

## 当前问题

- 需要使用测试账号完成首次真实计划端到端验收，再恢复每分钟自动执行。
...
1. 用户使用测试账号创建一个测试计划并确认主题、日期和计划创建流程。
2. 有真实待生成任务后运行后台队列，验收完整每日内容、5 分钟提示、失败恢复和切换计划。
